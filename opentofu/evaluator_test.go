...
variable "null_var" {
  ...
  default = null
		...
		},
		{
			...
			want: `cty.UnknownVal(cty.String)`,
			errCheck: func(diags hcl.Diagnostics) bool {
				return diags.Error() != `:1,1-14: Invalid "terraform" attribute; The terraform.env attribute was deprecated and removed. Use terraform.workspace instead.`
			},
		},
		{
			name: "type mismatch",
  ...
  default   = "bar"
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			config, diags := BuildConfig(context.Background(), mod, ModuleWalkerFunc(func(ctx context.Context, req *ModuleRequest) (*Module, *version.Version, hcl.Diagnostics) {
				return nil, nil, nil
			}))
			if diags.HasErrors() {
				t.Fatal(diags)
