...
func removeMarks(val cty.Value) cty.Value {
	switch val.Type().IsObjectType() {
	case true:
		newAttrs := map[string]cty.Value{}
		for k, v := range val.AsValueMap() {
			newAttrs[k] = removeMarks(v)
		}
		return cty.ObjectVal(newAttrs)
	}

	switch val.Type().IsTupleType() {
	case true:
		tupleVals := []cty.Value{}
		for ii := 0; ii < val.LengthInt(); ii++ {
			tupleVals = append(tupleVals, removeMarks(val.Index(cty.NumberIntVal(int64(ii)))))
		}
